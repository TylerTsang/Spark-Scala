object Practice01 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(79); 
...
// Data types exploration:
  ...
  val smallNumber : Byte = 123;System.out.println("""smallNumber  : Byte = """ + $show(smallNumber ));$skip(112); 
  
  
 // Building strings using concatination:
  println("This is a random string: " + number + truth + hello);$skip(81); 
  
  
 // Doubling pi then printing to 3 decimal places:
  val doublePi = pi * 2;System.out.println("""doublePi  : Double = """ + $show(doublePi ));$skip(55); 
  println(f"The double of pi is about: $doublePi%.3f");$skip(80); 
